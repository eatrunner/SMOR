...
void clImageProcessing1(unsigned char *image, unsigned char *out_image, int w, int h)
{
...
void clImageProcessing2(unsigned char *image, unsigned char *out_image, int w, int h)
{
	int tmp_shifts[] = { -1 - w, -w, -w + 1, -1, 0, 1, -1 + w, w, 1 + w };
	std::vector<float> vx(w*h); //wektory odpowiedajace macierzy
	std::vector<float> vy(w*h);
...
void clImageProcessing3(unsigned char *image, unsigned char *out_image, int w, int h)
{
	int tmp_shifts[] = { -1 - w, -w, -w + 1, -1, 0, 1, -1 + w, w, 1 + w };
	std::vector<float> vx(w*h); //wektory odpowiedajace macierzy
	std::vector<float> vy(w*h);
...
int main(int argc, char* argv[])
{
	...
	{
		clImageProcessing1(in_ptr[i], out_ptr[3*i], width, height);
		clImageProcessing2(in_ptr[i], out_ptr[3*i + 1], width, height);
		clImageProcessing3(in_ptr[i], out_ptr[3*i + 2], width, height);
	}
